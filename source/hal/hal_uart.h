...
// Opaque pointer for the UART device handle
...
typedef struct hal_uart_driver_s
{
    void (*init)(void);
    void (*deinit)(void);
    hal_uart_dev_t (*open)(hal_uart_port_t port, hal_uart_config_t* cfg);
    ...
    // CORRECTED: Added const to the buffer to ensure type safety.
    int32_t (*write)(hal_uart_dev_t dev, const uint8_t* buffer, size_t size);
    void (*flush)(hal_uart_dev_t dev);
} hal_uart_driver_t;

void hal_uart_init(void);
void hal_uart_deinit(void);
hal_uart_dev_t hal_uart_open(hal_uart_port_t port, hal_uart_config_t* cfg);
...
// CORRECTED: Added const to the buffer to match the driver definition.
int32_t hal_uart_write(hal_uart_dev_t dev, const uint8_t* buffer, size_t size);
